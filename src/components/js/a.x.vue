...
解释a.x:如果变量是之前已有内存（堆{n:1}）的属性（不管之前有没有存在，没有存在就创建然后指向undefined先），然后用该堆中对象的该属性指向{n:2},刚好说明了b.x为什么也指向{n:2},因为恰好b也指向该堆中的内存，而a.x为undefined因为执行a.x=a={n:2}的时候，a.x根本就不是操作a执行新的内存{n:2}的属性,所以a.x为undefine,如下图
